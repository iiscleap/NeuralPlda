...
def dataloader_from_sre08_10_trial(trials, enroll_model2xvector, test_xvectors, utts_dict, batch_size=2048,
                                   shuffle=True):
    x1_arr, x2_arr = [], []
    y_arr = []
...
def generate_scores_from_plda(mean_vec_path, transform_mat_path, plda_path, score_file_path, trials_path,
                              xvector_scp_path, spk2utt_path, num_utts_path):
    subprocess.call(
...
def load_xvec_from_batch(mega_xvec_dict, num_to_id_dict, data1, data2, device):
    ...
    for i, (d1, d2) in enumerate(zip(data1, data2)):
        data1_xvec_temp, data2_xvec_temp = mega_xvec_dict[num_to_id_dict[int(d1)]], mega_xvec_dict[
            num_to_id_dict[int(d2)]]
...
def dataloader_from_trials_list(trial_file_paths_list, enroll_spk2xvectors_list, test_xvectors_list, batch_size=2048,
    ...
    x1_arr, x2_arr = [], []
    y_arr = []
    for trial_file_path, enroll_spk2xvector, test_xvectors in zip(trial_file_paths_list, enroll_spk2xvectors_list,
                                                                  test_xvectors_list):
