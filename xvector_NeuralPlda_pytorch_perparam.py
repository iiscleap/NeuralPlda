...
class NeuralPlda(nn.Module):
    def __init__(self, xdim=512, LDA_dim=170, PLDA_dim=170, device=torch.device("cuda")):
        ...
        self.threshold2 = nn.Parameter(0*torch.rand(1, requires_grad=True)) # nn.Parameter(torch.tensor(5.2933)).to(device)
...
def train_each_param(args, model, device, train_loader, valid_loader, mega_xvec_dict, num_to_id_dict, optimizer_dict, epoch, ):
    model.eval()
    minC_threshold1, minC_threshold2, min_cent_threshold = compute_minc_threshold(args, model, device, mega_xvec_dict,
                                                                                  num_to_id_dict, valid_loader)
    model.train()
...
def main_kaldiplda():
    ...
    logging.info(
        "Started at {}\n\n SoftCdet Loss. Kaldi PLDA init. Batch size = 4096. Threshold1 and Threshold2 fixed using SRE 2018 dev after kaldi init. \n\n".format(
...
#    optimizer = optim.Adam(model.parameters(), lr=lr)
    optimizer_dict = {param:optim.Adam([param_dict[param]], lr=lr) for param in param_dict.keys()}
    all_losses = []
...
#        train(args, model, device, train_loader, valid_loader, mega_xvec_dict, num_to_id_dict, optimizer, epoch)
        train_each_param(args, model, device, train_loader, valid_loader, mega_xvec_dict, num_to_id_dict, optimizer_dict, epoch)
        ...
        try:
            ...
            if (all_losses[-1] > all_losses[-2]) and (all_losses[-2] > all_losses[-3]):
                ...
                for param in optimizer_dict.keys():
                    optimizer_dict[param] = optim.Adam(model.parameters(), lr=lr)
        except:
            pass
